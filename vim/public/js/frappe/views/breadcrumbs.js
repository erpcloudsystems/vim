...
frappe.breadcrumbs = {
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (breadcrumbs.workspace) {
			let workspace = breadcrumbs.workspace; 
			if(breadcrumbs.workspace == "HR" || breadcrumbs.workspace == "Payroll")
				workspace = "HRM";
			if (!breadcrumbs.module_info.blocked && frappe.visible_modules.includes(breadcrumbs.module_info.module)) {
				$(`<li><a href="/app/${frappe.router.slug(workspace)}">${__(workspace)}</a></li>`)
